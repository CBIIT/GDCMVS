...
def check_connection(client):
    """Check if above command succeeded or not by printing client info, 
       which will print cluster name and version if connection is successful
    """
    print(client.info())


def delete_gdc_suggestion_index(client):
    """Deletes the gdc-suggestion index if it exists."""
    index='gdc-suggestion',
    if client.indices.exists(index=index):
      response = client.indices.delete(
        index='gdc-suggestion',
      )
      print('Deleting index gdc-suggestion:', response)


def delete_gdc_p_index(client):
    """Deletes the gdc-p index if it exists."""
    index='gdc-p',
    if client.indices.exists(index=index):
      response = client.indices.delete(
        index='gdc-p',
      )
      print('Deleting index gdc-p:', response)
...
if __name__ == '__main__':
    ...
    delete_gdc_p_index(client)
    delete_gdc_suggestion_index(client)
