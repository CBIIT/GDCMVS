...
let tmpl = '<div class="container table__container"><div class="table__thead row">'
  ...
  +'<div class="col-xs-9">'
    +'<div class="table__thead row">'
      +'<div class="table__th col-xs-6">Matched GDC Values <a class="table__tooltip tooltip-target" data-toggle="tooltip" data-placement="bottom" title="Values that are found in the GDC dictionary and may be successfully submitted for the corresponding property."><i class="fa fa-info-circle"></i></a></div>'
      +'<div class="table__th col-xs-6">CDE Permissible Values <a class="table__tooltip tooltip-target" data-toggle="tooltip" data-placement="bottom" title="For GDC dictionary properties that have a corresponding caDSR clinical data element (CDE), these values are part of that CDE\'s value domain in the caDSR. They may not currently be available in the GDC dictionary."><i class="fa fa-info-circle"></i></a></div>'
    +'</div>'
  +'</div>'
+'</div>'
...
let tmpl = '{{for options}}<div class="suggest__object">'
			+'<span class="suggest__name">{{:id}}</span>'
			+'<label class="suggest__type">{{:type}}</label>'
			+'</div>{{/for}}';
...
const func = {
  ...
  toCompare(uid){
  	__WEBPACK_IMPORTED_MODULE_1__api__["a" /* default */].getGDCDataById(uid, function(id, items) {
        ...
        let tp = (window.innerHeight * 0.2 < __WEBPACK_IMPORTED_MODULE_2__shared__["a" /* default */].headerOffset() )? 20 : window.innerHeight * 0.2;
...
function gdcData(prop, item) {
  __WEBPACK_IMPORTED_MODULE_1__api__["a" /* default */].getGDCDataById(prop, function(id, items) {
    ...
    let header = $.templates(__WEBPACK_IMPORTED_MODULE_0__view__["a" /* default */].header).render({target: target, icdo: icdo, items_length: items.length});
    let html = $.templates(__WEBPACK_IMPORTED_MODULE_0__view__["a" /* default */].body).render({target: target, icdo: icdo, items: items});
    let tp = (window.innerHeight * 0.2 < __WEBPACK_IMPORTED_MODULE_2__shared__["a" /* default */].headerOffset() )? 20 : window.innerHeight * 0.2;
...
function getGDCTerms(uid, tgts){
  __WEBPACK_IMPORTED_MODULE_1__api__["a" /* default */].getGDCDataById(uid, function(id, items){
    ...
    let header = $.templates(__WEBPACK_IMPORTED_MODULE_0__view__["a" /* default */].header).render({targets: targets, icdo: icdo, items_length: items.length })
    let html = $.templates({markup: __WEBPACK_IMPORTED_MODULE_0__view__["a" /* default */].body, allowCode: true}).render({targets: targets, icdo: icdo, items: items });
    let tp = (window.innerHeight * 0.2 < __WEBPACK_IMPORTED_MODULE_2__shared__["a" /* default */].headerOffset() )? __WEBPACK_IMPORTED_MODULE_2__shared__["a" /* default */].headerOffset() + 20 : window.innerHeight * 0.2;
...
function cdeData(uid, tgts){
  __WEBPACK_IMPORTED_MODULE_1__api__["a" /* default */].getCDEDataById(uid, function(id, items) {
    ...
    let header = $.templates(__WEBPACK_IMPORTED_MODULE_0__view__["a" /* default */].header).render({targets: targets, items_length: items.length });

    let html = $.templates({markup: __WEBPACK_IMPORTED_MODULE_0__view__["a" /* default */].body, allowCode: true}).render({targets: targets, items: tmp });
    let tp = (window.innerHeight * 0.2 < __WEBPACK_IMPORTED_MODULE_2__shared__["a" /* default */].headerOffset() )? __WEBPACK_IMPORTED_MODULE_2__shared__["a" /* default */].headerOffset() + 20 : window.innerHeight * 0.2;
...
function ncitDetails(uid){
  __WEBPACK_IMPORTED_MODULE_1__api__["a" /* default */].evsRestApi(uid, function(id, item) {
    ...
    let windowEl = $(window);
    let tp = (window.innerHeight * 0.2 < __WEBPACK_IMPORTED_MODULE_2__shared__["a" /* default */].headerOffset() )? __WEBPACK_IMPORTED_MODULE_2__shared__["a" /* default */].headerOffset() + 20 : window.innerHeight * 0.2;
    let header = $.templates(__WEBPACK_IMPORTED_MODULE_0__view__["a" /* default */].header).render();
    let html = $.templates(__WEBPACK_IMPORTED_MODULE_0__view__["a" /* default */].body).render({item: tmp});
